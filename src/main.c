...
int main(int argc, char *argv[])
{
    ...
    // cut edges optimization
    // cut_edges_optimization(&graph, &partition_data, max_edges_cut);
    ...
    for (int i = 0; i < parts; i++)
    {
        free(partition_data.parts[i].part_vertexes); // Zwalniamy tablicę wierzchołków
    }
    free(partition_data.parts); // Zwalniamy tablicę części
